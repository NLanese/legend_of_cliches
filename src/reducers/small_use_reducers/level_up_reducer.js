    ...
    switch(action.type){
        ...
        case("ADD_POINT"):
            if (action.payload == "atk"){
                return {...state, pointsRemaining: state.pointsRemaining - 1, spd: state.atk + 1}
            }
            else if (action.payload == 'sAtk'){
                return {...state, pointsRemaining: state.pointsRemaining - 1, spd: state.sAtk + 1}
            }
            else if (action.payload == 'def'){
                return {...state, pointsRemaining: state.pointsRemaining - 1, spd: state.def + 1}
            }
            else if (action.paylaod == 'sDef'){
                return {...state, pointsRemaining: state.pointsRemaining - 1, spd: state.sDef + 1}
            }
            else if (action.payload == "spd"){
                return {...state, pointsRemaining: state.pointsRemaining - 1, spd: state.spd + 1}
            }
            else if (action.payload == "hp"){
                return {...state, pointsRemaining: state.pointsRemaining - 1, hp: state.hp + 5}
            }
